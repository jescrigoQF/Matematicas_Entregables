...
scalar n1 numero de escenarios /51/;
SCALAR n2 numero de activos /5/;
SCALAR M OBJETIVO MEDIO DEL RENDIMIENTO /0.2/;
SCALAR P EQUIPROBABILIDAD EN LOS ESTADOS /0.02/;
...
PARAMETER B(T,J) TASAS DE RENDIMIENTO;
B(T,J)=1*C(T,J);

SET I /t-0*t-49 /;

PARAMETER R(I,J)
loop((I(T),J),R(I,J)=100*(B(T+1,J)-B(T,J))/B(T,J));
...
EQUATIONS FO, RESTRICT1, RESTRICT2, RESTRICT3(I);

FO.. LOWVAR=E=P*SUM(I, POWER(Q(I),2));
RESTRICT1.. SUM(J,MEDIA(J)*X(J))=G=M;
RESTRICT2.. SUM(J,X(J))=e=1;
RESTRICT3(I).. SUM(J,(R(I,J)*X(J)))+Q(I)=G=M;

MODEL EJERCICIO1 /ALL/;

SOLVE EJERCICIO1 USING NLP MINIMIZING LOWVAR;
DISPLAY X.L, LOWVAR.L;
